...
export default function AssetAddComponent() {
  ...
  const handleSaveAsset = () => {
    if (selectedFile) {
      ...
      FileUploadService.storeFile(formData, localStorage.getItem("token")).then(res => {
        if (res.data.success) {
          AssetService.saveAsset({ ...newItem, image: res.data.payload }).then(res => {
            if (res.data.success) {
              setNewItem({
                name: "",
                image: "",
      ...
      AssetService.saveAsset(newItem).then(res => {
        if (res.data.success) {
          setNewItem({
            name: "",
            image: "",
              ...
              <section className="">
                ...
                <div className="flex w-full gap-x-6 gap-y-3 flex-col lg:flex-row my-3">
                  <div className='text-left w-full'>
                    ...
                    <textarea onInput={handleInputChange} name="description" rows="5" className="text-sm border-[#E1E9EE] placeholder-black placeholder-opacity-20  w-full border rounded-lg block py-2 px-4 mb-1 outline-none" placeholder="Enter description"></textarea>
                  </div>
                </div>
              </section>
            </div>
                    ...
                    <div className='w-full min-h-[14rem] flex p-5 relative'>
                      {
                        selectedFile ? <img src={URL.createObjectURL(selectedFile)} className='h-full m-auto' />
                          : <img className="block m-auto h-32" src={placeholder} />
                      }
                    </div>
                    <p className='mt-5 text-xs'>Upload Asset Picture</p>
                    <br />
