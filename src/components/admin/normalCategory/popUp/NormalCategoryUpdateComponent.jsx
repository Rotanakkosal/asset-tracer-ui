...
import { setIsUpdateSuccess, setNormalCategory } from "../../../../redux/slices/NormalCategorySlice";
import { Slide, ToastContainer, toast } from "react-toastify";

export default function NormalCategoryUpdateComponent() {
    ...
    const getNormalCategory = useSelector(state => state.normalCategory.normalCategory)
    ...
    const formik = useFormik({
        ...
        onSubmit: (val, { resetForm }) => {
            handleUpdateNormalCategory()
        },
    });

    useEffect(() => {
        setNewData({ ...getNormalCategory })
        formik.setValues(getNormalCategory)
        setSelectedSuperCategory({ name: getNormalCategory.superCategoryName })
        if (getNormalCategory.icon) {
            setSelectedImg(getNormalCategory.icon)
        }
    }, [getNormalCategory])
    ...
    // Add Super Category
    const handleUpdateNormalCategory = () => {
        setIsLoading(true)
        if (selectedFile != "laptop.svg" && selectedFile != "paper.svg" && selectedFile != "chair.svg" && selectedFile != "") {
            formData.append("file", selectedFile)
            FileUploadService.storeFile(formData, localStorage.getItem("token"))
                .then((res) => {
                    if (res.data.success) {
                        NormalCategoryService.updateNormalCategory(
                            ...
                            if (res.data.success) {
                                setTimeout(() => {
                                    setIsLoading(false)
                                    document.getElementById("edit-normal-category").click();
                                    setSelectedFile("")
                                    setSelectedSuperCategory("")
                                    dispatch(setIsUpdateSuccess(true))
            ...
            NormalCategoryService.updateNormalCategory(getNormalCategory.id, newData).then((res) => {
                if (res.data.success) {
                    setTimeout(() => {
                        setIsLoading(false)
                        document.getElementById("edit-normal-category").click();
    ...
    return (
        <div>
            ...
            <input type="checkbox" id="edit-normal-category" className="modal-toggle" />
            <div className=" modal">
                    ...
                    <form onSubmit={formik.handleSubmit}>
                        <div className=" border border-bg-primary rounded-md p-2 my-6">
                            <div className="flex items-center gap-3">
                                <label>
                                    {selectedFile ?
                                        <img
                                            src={selectedFile == "laptop.svg" ? laptop : selectedFile == "paper.svg" ? paper : selectedFile == "chair.svg" ? chair : URL.createObjectURL(selectedFile)}
                                            className="h-14 rounded-full w-14 "
                                        />
                                        : selectedImg ?
                                            <img
                                                src={selectedImg == "laptop.svg" ? laptop : selectedImg == "paper.svg" ? paper : selectedImg == "chair.svg" ? chair : FileUploadService.getImage(selectedImg)}
                                                className="h-14 rounded-full w-14 "
                                            />
                                            :
                                            <img
                                    ...
                                    <div className="flex flex-row items-end justify-start">
                                        ...
                                        <div className="p-2">
                                            <label className="cursor-pointer" onClick={() => handleSelectedFile("paper.svg")}>
                                                <img src={paper} className="h-8 rounded-lg w-8 " />
                                            </label>
                                        </div>
                                        <div className="p-2">
                        ...
                        <div className="flex justify-end flex-row space-x-4 mt-10">
                            ...
                            <div className="">
                                ...
                                >
                                    <FiSave className="text-lg" /> &nbsp; Save change
                                </button>
                            </div>
                        </div>
                    </form>
